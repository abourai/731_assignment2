...
class IBM():
  def __init__(self, bitext, max_iter = 7):
	...
	for (e,f) in self.bitext:
		...
		if len(e) > self.max_e: self.max_e = len(e)
  ...
  def train(self):
	...
	for num in range(self.max_iter):
	 ...
	 # print count
		...
		count_q = defaultdict(lambda: 0.0)
		total_q = defaultdict(lambda: 0.0)
		...
		#	total_f = defaultdict(float)
			for j in xrange(len(f)):
				...
				for i in xrange(len(e)):
					...
					count_q[j,i,len(e),len(f)] += self.theta[e[i], f[j]] / total_e[e[i]]
					total_q[i,len(e),len(f)]   += self.theta[e[i], f[j]] / total_e[e[i]]
...
#--------------ibm2-------------------
	self.q = defaultdict(lambda: initial_prob)
	for num in range(self.max_iter):
		count = defaultdict(lambda: 0.0)
		total_f = defaultdict(lambda: 0.0)
		count_q = defaultdict(lambda: 0.0)
		...
		#	print f
			...
			for j in xrange(len(f)):
				for i in xrange(len(e)):
					total_e[e[i]] += self.theta[e[i], f[j]] * self.q[j,i,len(e),len(f)]
		#	total_f = defaultdict(float)
			for j in xrange(len(f)):
				for i in xrange(len(e)):
					count[e[i], f[j]] += self.theta[e[i], f[j]] * self.q[j,i,len(e),len(f)]/ total_e[e[i]]
					total_f[f[j]]  += self.theta[e[i], f[j]] * self.q[j,i,len(e),len(f)]/ total_e[e[i]]
					count_q[j,i,len(e),len(f)] += self.theta[e[i], f[j]] * self.q[j,i,len(e),len(f)] / total_e[e[i]]
					total_q[i,len(e),len(f)]   += self.theta[e[i], f[j]] * self.q[j,i,len(e),len(f)] / total_e[e[i]]
		# (2) [M] θ[i,j] =  C[i,j] / Σ_j C[i,j] (Equation 107)
		# for (e,f) in self.bitext:
		# 	for f_word in f:
		# 		for e_word in e:
		...
		for (e_word, f_word) in self.theta.keys():
			self.theta[e_word, f_word] = count[e_word, f_word] / total_f[f_word]
		for l in range(self.max_e):
			for m in range(self.max_f):
				for j in range(m):
					for i in range(l):
						if total_q[i,l,m] != 0:
							self.q[j,i,l,m] = count_q[j,i,l,m] / total_q[i,l,m]
